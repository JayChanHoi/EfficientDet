...
def _get_detections(dataset, model, score_threshold=0.5, iou_threshold=0.5, max_detections=100):
    ...
    with torch.no_grad():
        for index in range(len(dataset)):
            ...
            if torch.cuda.is_available():
                scores, labels, boxes = model.module(
                    torch.from_numpy(data['image']).permute(2, 0, 1).cuda().float().unsqueeze(dim=0),
                    train=False,
                ...
                scores, labels, boxes = model.module(
                    torch.from_numpy(data['image']).permute(2, 0, 1).float().unsqueeze(dim=0),
                    train=False,
                    score_threshold=score_threshold,
                    iou_threshold=iou_threshold
                )
...
def evaluate(
    ...
    print('\nscore threshold {}; iou threshold {} mAP:'.format(score_threshold, iou_threshold))
    avg_mAP = []
    for label in range(generator.dataset.num_classes()):
